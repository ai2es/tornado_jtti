...
def make_directory_structure(this_storm_mask_dir):
    ...
    try:
        ...
        print(f"\nMake directory {data_dirpath} [dry_run={dry_run}]")
        if not dry_run: 
...
def main():
    ...
    print(f"all_storm_mask_dirs ({len(all_storm_mask_dirs)})", all_storm_mask_dirs)
    
    #find which day corresponds to index_primer
    this_storm_mask_dir = all_storm_mask_dirs[int(index_primer)]

    #pull out the date in YYYYMMDD format
    print("this_storm_mask_dir", this_storm_mask_dir)
    path_parts = this_storm_mask_dir.split('/')
    print("path_parts", path_parts)
    ...
    for mask_time in all_masks_day:
        ...
        if(last_hr != HH and last_hr != '-1' and len(output_nt_list) != 0):
            ...
            print(f"Saving L0: nt {patches_fname} [dry_run={dry_run}]")
            if not dry_run: 
            ...
            if(output_t_list != []):
                ...
                print(f"Saving L1: tor {patches_fname} [dry_run={dry_run}]")
                if not dry_run: 
                ...
                if(output_st_list != []):
                    ...
                    print(f"Saving L2: sigtor {patches_fname} [dry_run={dry_run}]")
                    if not dry_run: 
        ...
        # Read the storm mask
        storm_mask_fnpath = os.path.join(storm_mask_path, f'{YYYY}/{YYYYMMDD_path}/5_min_window/storm_mask_{YYYYMMDD_day_with_dashes}-{HHMMSS}.nc')
        print(f"open {storm_mask_fnpath}")
        storm_mask_and_metadata = xr.open_dataset(storm_mask_fnpath)
        ...
        # Open corresponding radar file
        radar_dirpath = os.path.join(input_radar_directory_path, f'{YYYY}/{YYYYMMDD_path}/nexrad_3d_v4_2_{YYYYMMDD_day}T{HHMMSS}Z.nc')
        print(f"open {radar_dirpath}")
        radar = xr.open_dataset(radar_dirpath)
        ...
        if(np.count_nonzero(storm_mask) > 0):
            ...
            # Identify x,y indices of all tornadic images
            ...
            print("possible_starting_indices", possible_starting_indices.shape)
            
            # Reduce array size by 'size' in both dimensions
            #possible_x_max, possible_y_max = (np.array(possible_starting_indices.shape) - np.ones(2)).astype(int)
            possible_x_max, possible_y_max = np.array(possible_starting_indices.shape, int) - np.ones(2)
            
            # Extract indices of all torandic pixels
            tornado_x, tornado_y = np.nonzero(storm_mask)
            
            # Find all pixels such that if this pixel was in the corner of the patch, the patch would contain a tornadic pixel
            ...
            # Find all indices where this is the case
            ...
            if(np.count_nonzero(storm_mask == 2) > 0):
                ...
                # Find all pixels such that if this pixel was in the corner of the patch, the patch would contain a sigtor pixel
